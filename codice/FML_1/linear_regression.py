...
class LinearRegression:
    ...
    def __init__(self, learning_rate=1e-2, n_steps=2000, n_features=1):
        ...
        self.theta = np.random.rand(n_features) #the theta value are set initialy to random value
    ...
    def fit(self, X, y):
        ...
        for step in range(0, self.n_steps):
            ...
            self.theta = self.theta - (self.learning_rate * (1/m) * np.dot(X.T, error))# since deltaJ(\theta)=X^T(X\theta-y)
